...
def preprocessing(dataset: pd.DataFrame):
  ...
  text_data = df.apply(lambda row: row.headline + '. ' + row.short_description, axis=1)
  ...
  # Lemmatizing
  text_data = [[WordNetLemmatizer().lemmatize(word, 'n') for word in text] for text in text_data]
  # Convert tokens to sentences
  text_data = [TreebankWordDetokenizer().detokenize(text) for text in text_data]
  return text_data
...
def split_dataset(data_text: pd.DataFrame, y: pd.core.series.Series):
  ...
  x_train, x_test, y_train, y_test = train_test_split(data_text, y, test_size=0.25, random_state=0)
  return x_train, x_test, y_train, y_test

@st.cache
def generate_tsne_visualization(tfidf: sp.sparse.csr.csr_matrix):
  tsne_tfidf = TSNE(n_components=2, learning_rate=100, perplexity=40).fit_transform(tfidf)
...
def plot_confusion_matrix(classifier, X_test, y_test):
  disp = ConfusionMatrixDisplay.from_estimator(classifier, X_test, y_test, display_labels=classifier.classes_, cmap=plt.cm.Blues)
...
def classificate_rf(tfidf_train: sp.sparse.csr.csr_matrix, tfidf_test: sp.sparse.csr.csr_matrix, labels_names):
  ...
  rf_clf = RandomForestClassifier(criterion='gini', max_depth=None, max_features='log2', n_estimators=300, random_state=0)
  # Apply Random Forest
...
st.write("Para classificação iremos utilizar apenas as colunas ['headline', 'short_description']")

plt.title('Amount of News Based On Category')
plt.xticks(rotation=90)
fig = sns.countplot(data=raw_df, x='category')
st.pyplot(fig.figure)
